...
import { ArrowLeft, Send, Upload, FileText, Trash2, InfoIcon, AlertTriangleIcon } from "lucide-react"
  ...
  const checkOllama = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (!data.available) {
          setMessages((prev) => [
            ...
            {
              role: "system",
              content: "Running in demo mode with mock responses. Start Ollama for full functionality.",
              timestamp: new Date(),
            },
          ])
        } else {
          setMessages((prev) => [
            ...
            {
              role: "system",
              content: `Connected to Ollama. Using ${data.completionModel} for responses.`,
              timestamp: new Date(),
            },
          ])
        }
        ...
        setMessages((prev) => [
          ...
          {
            role: "system",
            content: "Could not verify Ollama status. Running in demo mode with mock responses.",
            timestamp: new Date(),
          },
        ])
      }
      ...
      setMessages((prev) => [
        ...
        {
          role: "system",
          content: "Could not connect to Ollama. Running in demo mode with mock responses.",
          timestamp: new Date(),
        },
      ])
    }
...
const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (newUploadedFiles.length > 0) {
    ...
    setMessages((prev) => [
      ...
      {
        role: "system",
        content: `Uploaded ${newUploadedFiles.length} lesson plan(s): ${newUploadedFiles.map((f) => f.name).join(", ")}`,
        timestamp: new Date(),
      },
    ])
  }
    ...
    <div className="flex flex-col min-h-screen">
      <header className="px-4 lg:px-6 h-16 flex items-center border-b">
        <Link href="/" className="flex items-center justify-center">
          ...
          <span className="ml-2 text-sm text-gray-500">Lesson Plan Assistant</span>
        </Link>
      ...
      <main className="flex-1 container py-6 md:py-12 grid grid-cols-1 md:grid-cols-3 gap-6">
        {ollamaStatus.checking ? (
          <Alert className="md:col-span-3 mb-0">
            ...
            <AlertDescription>
              Checking connection to Ollama. This will only take a moment...
            </AlertDescription>
          </Alert>
        ) : !ollamaStatus.available ? (
          <Alert className="md:col-span-3 mb-0" variant="warning">
            <AlertTriangleIcon className="h-4 w-4" />
            <AlertTitle>Ollama Not Available</AlertTitle>
            <AlertDescription>
              Running in demo mode with mock responses. For full functionality, please start Ollama on your local machine.
            </AlertDescription>
          </Alert>
        ) : (
          <Alert className="md:col-span-3 mb-0" variant="default">
            ...
            <AlertDescription>
              Using {ollamaStatus.completionModel} for responses.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="md:col-span-2">
          <Card className="h-full flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                Ask questions about your lesson plans and their compliance with Innova Schools Mexico guidelines
              </CardDescription>
            </CardHeader>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Upload your lesson plans for analysis</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                ...
                <p className="text-sm text-gray-500">Upload PDF, DOCX, or TXT files</p>
              </div>

              {uploadedFiles.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-sm font-medium">Uploaded Lesson Plans:</h4>
                  <ul className="space-y-2">
                    {uploadedFiles.map((file) => (
                      <li key={file.id} className="flex items-center justify-between rounded-lg border p-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Enter Innova Schools Mexico guidelines for reference</CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea
              ...
              <ul className="space-y-2">
                <li>
                  <Button
                    ...
                    onClick={() => {
                      setInput("Does my lesson plan comply with Innova Schools guidelines?")
                    }}
                  >
                ...
                <li>
                  <Button
                    ...
                    className="text-left justify-start p-0 h-auto"
                    onClick={() => {
                      setInput("How can I improve the student engagement in this lesson?")
                    }}
                  >
                ...
                <li>
                  <Button
                    ...
                    className="text-left justify-start p-0 h-auto"
                    onClick={() => {
                      setInput("What assessment methods would work best for this lesson?")
                    }}
                  >
                ...
                <li>
                  <Button
                    ...
                    className="text-left justify-start p-0 h-auto"
                    onClick={() => {
                      setInput("How can I make this lesson more project-based?")
                    }}
                  >
      ...
      <footer className="border-t py-6 md:py-0">
        <div className="container flex flex-col items-center justify-between gap-4 md:h-24 md:flex-row">
          ...
          <p className="text-center text-xs text-gray-500">
            {ollamaStatus.available 
              ? `Powered by Ollama (${ollamaStatus.completionModel})` 
